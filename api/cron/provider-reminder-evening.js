// Day-before evening — texts each assigned cleaner the comprehensive
// details for tomorrow's job. One SMS per job.
//
// Schedule (vercel.json): daily at 23:00 UTC (7pm ET — evening of the
// day before each scheduled job).
...
const SENT_PREFIX = "provider-evening:";
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const tomorrowMidnightMs = new Date(target + "T00:00:00Z").getTime();
    const dayAfterMs = tomorrowMidnightMs + 86400000;
    const opps = (search?.opportunities || []).filter((o) => {
      const dt = getCfRaw(o.customFields, OPP_APPOINTMENT_DATE);
      if (!dt) return false;
      const t = new Date(dt).getTime();
      return t >= tomorrowMidnightMs && t < dayAfterMs;
    });
    ...
    for (const opp of opps) {
      try {
        ...
        // Fetch full contact (with customFields) + recent notes
        ...
        // Idempotency: tag the OPP (not the contact) so multiple jobs
        // for the same contact each get their own send. Use the per-day
        // suffix so the same opp can be re-pinged if it slips a day.
        const tags = (contact.tags || []).map((t) => String(t).toLowerCase());
        // We could check tags on the opportunity itself, but opp tags
        // aren't a thing in GHL — fall back to contact-level tag with a
        // job-specific suffix to prevent duplicate sends to the same job.
        ...
        const message = buildProviderSms({
          ...
          intro: "Tomorrow's clean",
        });

        // Send SMS via internal line — provider replies should stay
        // separate from the customer line conversations inbox.
        // We can't directly send to an arbitrary phone via /conversations
        // without a contactId, so we upsert the provider as a contact.
        const upsert = await ghl({
          ...
          body: {
            ...
            lastName: String(providerName).split(/\s+/).slice(1).join(" ") || undefined,
        ...
        // Tag the CUSTOMER contact so we don't double-send for this same opp
        await ghl({
          method: "POST",
